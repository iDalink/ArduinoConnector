...
void processCommander() {
  ...
  if (command == 0x50) {
    ...
    //设置读模式
    pinMode(pin, INPUT);
    unsigned char *string = "Pin   setRead";
    processString(string, pin);
  ...
  if (command == 0x70) {
    ...
    //设置写模式
    pinMode(pin, OUTPUT);
    unsigned char *string = "Pin   setWrite";
    processString(string, pin);
    displayString(string);
    delay(1000);
    ack();
